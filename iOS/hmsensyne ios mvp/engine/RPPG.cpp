...
void RPPG::processFrame(Mat &frameRGB, Mat &frameGray, int time) {
    try {
        // Set time
        this->time = time;
    //    cout << " " << this->timeBase << " " << this->time << " " << lastScanTime << endl;

        if (!faceValid) {

    //        std::cout << "Not valid, finding a new face" << endl;
    ...
    //        std::cout << "Valid, but rescanning face" << endl;
...
//        this->load(
//                   this->rPPGAlg,this->faceDetAlg, this-> width, const int height, const double timeBase, const int downsample,
//                                   const double samplingFrequency, const double rescanFrequency,
//                                   const int minSignalSize, const int maxSignalSize,
//                                   const string &logPath, const string &haarPath,
//                                   const string &dnnProtoPath, const string &dnnModelPath,
...
//              cout << endl << "haar algorithm found the box:" << (int)xLeftBottom << " " << (int)yLeftBottom << " " << (int)(xRightTop) << " " << (int)(yRightTop) << endl;
...
char* RPPG::getInferenceTextFromEngine() {
    char* toReturn = strdup("");
    if (!faceValid) {
        toReturn = strdup("Unable to find a face to track");
    } else {
        toReturn = strdup("Face found, please wait until the algorithm converges");
        if (lastBpmValue != bpmValue) {
            char bpmRepresentation[200];
            sprintf(bpmRepresentation, "%2.2f", bpmValue);
            toReturn = strdup("");
        }
    }
    return toReturn;
